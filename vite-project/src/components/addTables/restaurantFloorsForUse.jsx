...
const RestaurantFloorForUse = ( {prevStep,nextStep}) => {
    ...
    const isTableNumberUnique = floors.every((floor) =>
      floor.shapes.every(
        (shape) =>
          ...
          shapes.findIndex(
            (s) => s.tableNumber === newShape.tableNumber && s.type === shapeType && newShape.tableNumber != '' && s.tableNumber != ''
          ) === -1
      )
    );
          ...
          <label style={{ maxWidth: '300px' }}>
            ...
            <Select
              ...
              options={tagOptions.map(option => ({
                ...option,
                isDisabled: selectedShapeIndex !== null, // Disable tags when a shape is selected
              }))}              isMulti
              styles={{
...
export default RestaurantFloorForUse;
